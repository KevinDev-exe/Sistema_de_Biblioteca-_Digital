...
class ReservaCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy("reservas:list" if self.request.user.es_bibliotecario else "reservas:mis_reservas")
...
def reservar_libro(request, libro_id):
    ...
    libro = get_object_or_404(Libro, pk=libro_id, activo=True)
    ...
    return redirect("libros:libro_detail", pk=libro.pk)
...
def cancelar_reserva(request, pk):
    ...
    if reserva.usuario != request.user and not request.user.es_bibliotecario:
    ...
    return redirect("reservas:list" if request.user.es_bibliotecario else "reservas:mis_reservas")
